...
{{ $k }} = host={{ $v.host }} port={{ $v.port | default "5432" }} {{ if $v.user }}user={{ $v.user }}{{end}} dbname={{ $root.Release.Namespace | replace "-" "_"}}_{{ required $requiredMsg $v.dbname }}
{{- else }}
{{ $k }} = host={{ $v.host }} port={{ $v.port | default "5432" }} {{ if $v.user }}user={{ $v.user }}{{end}} dbname={{ required $requiredMsg $v.dbname }} {{ if $v.max_db_connections }}max_db_connections={{ $v.max_db_connections }}{{end}}
{{- end }}
...
;tcp_socket_buffer = 0
tcp_keepalive = 1
tcp_keepcnt = {{ .Values.tcpKeepcnt }}
tcp_keepidle = {{ .Values.tcpKeepidle }}
tcp_keepintvl = {{ .Values.tcpKeepintvl }}
